...
function initCookies(){
	if(!Cookies.get("token") || Cookies.get("token") == "None"){
		$.ajax({
			  url: '/clinical-ui/assets/conf/environment.cfg?26',
			  dataType: 'json',
			  async: false,
...
function writeUserData(udata){
	$.ajax({
        ...
        error: function(){
             ...
             //window.location.replace("/clinical-ui/application/pages/login.html");
         }
    });
...
function resume_download(){
	...
	window.location.replace("/clinical-ui/download.html");
}
...
function download_files(formname){
    ...
    if (get_var["dataset_id"] && get_var["dataset_id"] != "undefined"){
	    Cookies.set("login_redirect", "/clinical-ui/d/index.html?dataset_id="+get_var["dataset_id"])
    }else if (get_var["search"]){
	    Cookies.set("login_redirect", "/clinical-ui/s/index.html?search="+get_var["search"])
    }
...
function reset_search_filters(){
	...
	window.history.replaceState({}, document.title, "/" + "clinical-ui/s/index.html?search=*");
}
